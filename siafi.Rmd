...
filtro_dois_vinculos <- tipo_vinculo %>% filter(q_vinculos == 2) %>% summarise(cpf_nome)
...
cadastro_cargo_dois_vinculos <- cadastro_total%>% filter( cpf_nome %in% c(filtro_dois_vinculos$cpf_nome, FUNCAO %in% c ("Sem informação", "Inativo") )) %>% select ( -FUNCAO,- SIGLA_FUNCAO, -NIVEL_FUNCAO)
